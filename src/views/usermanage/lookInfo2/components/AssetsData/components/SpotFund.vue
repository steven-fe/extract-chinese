<template>
  <div class="spotfund-container">
    ...
    <el-table v-loading="loading" border :data="tableData" style="width: 100%">
      <el-table-column prop="time" :label="$t('userManager.takeTime')">
      ...
      <el-table-column prop="typeDesc" :label="$t('userManager.fundType')" />
      <el-table-column prop="amount" :label="$t('userManager.price')" />
    </el-table>
...
<script>
import { getRecordTypeList, getSpotRecordList } from '@/api/user'
  ...
  methods: {
    ...
    async getData() {
      try {
        ...
        const { data } = await getSpotRecordList(this.queryParams)
    ...
    async getRecordTypeList() {
      try {
        const { data } = await getRecordTypeList('SPOT')
