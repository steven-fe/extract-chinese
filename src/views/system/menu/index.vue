  ...
  <div class="app-container">
    <el-form :inline="true">
      ...
      <el-form-item :label="$t('common.status')">
        <el-select v-model="queryParams.visible" placeholder="菜单状态" clearable size="mini">
          <el-option
...
export default {
  ...
  methods: {
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          if (this.form.parentId === 0) {
            this.form.component = 'Layout' // 一级目录
          } else {
            if (this.form.menuType === 'M') {
              this.form.component = 'log/index' // 二级目录正常展示
            }
          }
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      this.$confirm('是否确认删除名称为"' + row.menuName + '"的数据项?', this.$t('common.alert'), {
