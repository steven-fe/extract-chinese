<template>
  <div>
    <div class="search">
      <el-form ref="form" :inline="true" :model="form" label-width="123px">
        <el-form-item :label="$t('common.balanceIsLockedInCurrency')" prop="coin" label-width="215px">
    ...
    <div class="create">
      <el-button type="success" size="mini" icon="el-icon-plus" @click="importData">{{ $t('common.userbalanceIsLocked') }}</el-button>
    </div>
    <el-table :data="tableData" border style="width: 100%" class="table cabin-table" @selection-change="selectionChange">
      ...
      <el-table-column prop="coin" :label="$t('common.lockInCurrency')" align="center" />
      <el-table-column prop="amount" :label="$t('common.quantityOfLockUp')" align="center" />
      <el-table-column prop="ctime" :label="$t('financemanager.lockupTime')" align="center">
      ...
      <el-table-column prop="remark" :label="$t('common.lockupRemarks')" align="center" />
    </el-table>
    <!--分页组件-->
    <div class="block top">
      <pagination
    ...
    <import-data ref="importData" :title="$t('financemanager.airdropLockUpTokens')" :type="0" :close-on-click-modal="false" @success="initTableData">
      <div class="tips">
...
export default {
  ...
  methods: {
    selectionChange(arrRow) {
      ...
      console.log('选择的数据123', this.selectData)
    },
    initTableData() {
      console.log('上传参数123', this.form)
    ...
    exportData(flag) {
      if (flag === 1) {
        ...
        lockExport({ ids, type: 0 }).then(res => {
          if (res.code === 200) {
