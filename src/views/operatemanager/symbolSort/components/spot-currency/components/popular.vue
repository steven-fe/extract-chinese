  ...
  <div>
    <div :class="$style.serachBox">
      <el-select
        v-model="searchValue"
        filterable
        :placeholder="$t('symbolSort.selectCoinPairCode')"
        clearable
      >
        <el-option
      ...
      <div :class="$style.button">
        ...
        <el-tooltip :content="$t('symbolSort.upTo20Online')" placement="top">
      ...
      <template>
        <div id="app">
          <tree-transfer
            ...
            default-expand-all
            show-checkbox
            :titles="[$t('symbolSort.allCurrencyPairCodes'), $t('symbolSort.popularCoin')]"
    ...
    <el-dialog :title="$t('symbolSort.sort')" :visible.sync="sortVisible" :close-on-click-modal="false" width="600px">
      <div :class="$style.mb">{{ $t('symbolSort.maxCionLimite20') }}</div>
      ...
      <div :class="$style.footerButton">
        <span slot="footer" :class="$style.footerDf" class="dialog-footer" >
          ...
          <div>
            <el-tooltip placement="top">
              <div slot="content">{{ $t('symbolSort.maxSort20') }}<br/>{{ $t('symbolSort.adjustSort') }}<br/><br/>{{ $t('symbolSort.awaitUse') }}</div>
              <i :class="$style.iconInfo" class="el-icon-info" />
            </el-tooltip>
...
export default {
  ...
  methods: {
    async getList() {
      const { data } = await getHomeSymbolList({ symbol: this.searchValue, type: 2 })
      this.dataList = data
    },
    async handleEdit() {
      this.model = []
      const { data } = await getHomeSymbolList({ symbol: '', type: 2 })
      data.map(item => {
    ...
    sortSure() {
      ...
      sortCurrency({ type: 2, symbolList: sortArr }).then(res => {
        if (res.code === 200) {
