<template>
  <div class="app-container">
    ...
    <el-table :data="tableData" border style="width: 100%" class="table cabin-table" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="bonusGrantZhangNumber" width="200px" label="发放张数" align="center" />
      <el-table-column prop="bonusUseZhangNumber" width="200px" label="使用张数" align="center" />
      <el-table-column prop="bonusProfitZhangNumber" width="200px" label="盈利张数" align="center" />
      <el-table-column prop="bonusProfitAmount" width="200px" label="盈利金额(U)" align="center" />
      <el-table-column prop="bonusLossZhangNumber" width="200px" label="亏损张数" align="center" />
      <el-table-column prop="bonusLossAmount" width="200px" label="亏损金额(U)" align="center" />
    </el-table>
    <!--分页组件-->
    <div class="block top">
      <pagination
...
<script>
import { bonusPageList, bonusExportData, getSymbol } from '@/api/contract/contractDaily'
import { commonFilter } from '@/utils/filters'
import { date, time } from '@/utils/format'
import { mapState } from 'vuex'

export default {
  name: 'ContractBonusPageList',
  filters: {
    timeFilter(val) {
      if (Number(val) === 0) {
        return '- -'
  ...
  methods: {
    // 导出
    exportData() {
      bonusExportData(this.form).then(res => {
        if (res.code === 200) {
    ...
    getConfigList(flag) {
      ...
      bonusPageList(this.form).then(response => {
        if (response.code == 200) {
