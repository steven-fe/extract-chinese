<template>
  <div class="app-container">
    ...
    <el-table :data="tableData" border style="width: 100%" class="table cabin-table" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="todayFee" width="200px" label="当日手续费（U）" align="center" />
      <el-table-column prop="takerFee" width="200px" label="taker_fee(U)" align="center" />
      <el-table-column prop="makerFee" width="200px" label="maker_fee(U)" align="center" />
    </el-table>
    <!--分页组件-->
    <div class="block top">
      <pagination
...
<script>
import { profitAndLossPageList, profitAndLossExportData, getSymbol } from '@/api/contract/contractDaily'
import { commonFilter } from '@/utils/filters'
import { date, time } from '@/utils/format'
import { mapState } from 'vuex'

export default {
  name: 'ProfitAndLossPageList',
  filters: {
    timeFilter(val) {
      if (Number(val) === 0) {
        return '- -'
  ...
  methods: {
    // 导出
    exportData() {
      profitAndLossExportData(this.form).then(res => {
        if (res.code === 200) {
    ...
    getConfigList(flag) {
      ...
      profitAndLossPageList(this.form).then(response => {
        if (response.code == 200) {
