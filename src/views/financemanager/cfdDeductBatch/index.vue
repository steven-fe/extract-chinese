<template>
  <div class="app-container">
    <el-form
      ...
      label-width="75px"
    >
      <el-form-item label="划扣批次" prop="batchNo">
        <el-input
      ...
      <el-form-item :label="$t('dwmanager.userUID')" prop="fromUid">
        <el-input
    ...
    <el-table v-loading="loading" :data="transferList" border>
      <el-table-column
        ...
        width="180px"
      />
      <el-table-column :label="$t('dwmanager.userUID')" align="center" prop="fromUid" />
      <el-table-column label="划扣币种" align="center" prop="currency" />
      <el-table-column label="划扣数量" align="center" prop="amount" />
      <el-table-column label="划扣备注" align="center" prop="meta" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="600px">
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="客户ID和划扣数量" prop="deductInfo">
          <el-input
            v-model="form.deductInfo"
            type="textarea"
            :rows="5"
            placeholder="请按规则输入客户ID和划扣数量"
          />
        </el-form-item>
        <el-form-item label>
          <span
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" :disabled="disabled" @click="submitForm">确定划扣</el-button>
...
import {
  ...
  uploadTemplate,
  recallDeduct
} from '@/api/finance/financeDeductBatch'
...
export default {
  ...
  data() {
    return {
      ...
      uploadUrl:
        ...
        '/coinbull/assets/deduct/finance/importData',
      // 是否显示弹出层
      open: false,
      batchopen: false,
      disabled: false,
      ...
      rules: {
        ...
        meta: [
          { required: true, message: '划扣备注不能为空', trigger: 'blur' }
        ],
        airdropInfo: [
          {
            required: true,
            message: '客户ID和划扣数量不能为空',
            trigger: 'blur'
          }
        ]
      }
  ...
  methods: {
    ...
    submitUpload() {
      const that = this
      this.$confirm('是否确定要执行本次划扣操作吗?', this.$t('common.alert'), {
    ...
    handleRecall(row) {
      const that = this
      this.$confirm('是否确定要撤销本次划扣操作吗?', this.$t('common.alert'), {
        confirmButtonText: this.$t('common.sure'),
        cancelButtonText: this.$t('common.cancel'),
        type: 'warning'
      }).then(function() {
        recallDeduct(row).then((res) => {
          if (res.code === 200) {
