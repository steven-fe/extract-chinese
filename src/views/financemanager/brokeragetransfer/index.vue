<template>
  <div class="app-container">
    <el-form
      ...
      class="form-grid"
    >
      <el-form-item class="grid-item" :label="$t('contractmanager.feeAccount')" prop="fromUserId">
        <el-select v-model="digform.fromUserId" style="width: 100%" :placeholder="$t('common.pleaseChoose')">
          <el-option
      ...
      <el-form-item class="grid-item" :label="$t('financemanager.targetSystemAccount')" prop="toUserId">
        <el-select v-model="digform.toUserId" style="width: 100%" :placeholder="$t('common.pleaseChoose')">
          <el-option
      ...
      <el-form-item class="grid-item" :label="$t('common.coin')" prop="currencyId">
        <el-select v-model="digform.currencyId" style="width: 100%" :placeholder="$t('common.selectCurrency')" filterable>
          <el-option
...
import { brokerageUser, assetFromUser, brokerageAsset } from '@/api/finance/freecurrency'

export default {
  name: 'Brokeragetransfer',
  data() {
    ...
    return {
      ...
      rules: {
        fromUserId: [
          { required: true, message: this.$t('financemanager.pleaseSelectASystemAccount'), trigger: 'change' }
        ],
        toUserId: [
  ...
  mounted() {
    ...
    assetFromUser().then(res => {
      if (res.code === 200) {
        this.toAccountOptions = res.data
      }
    })
  },
  methods: {
      ...
      this.$refs['digform'].validate((valid) => {
        if (valid) {
          this.$confirm(this.$t('financemanager.whetherToTransferToTheAccount'), this.$t('common.prompt'), {
            ...
            brokerageAsset(this.digform).then(res => {
              if (res.code === 200) {
                this.$message({
                  type: 'success',
