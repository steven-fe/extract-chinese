...
import { linkTypes, clientTypes, showTypes, getLanguages } from '../../../../../../data'
...
export default {
    ...
    formItems() {
      return [
        ...
        },
        {
          prop: 'triggerTime',
          label: '推送时间',
          rules: [{ required: true, message: $t(`['common']['propTimeCannotNull']`) }],
        ...
        },
        {
          ...
          rules: [{ required: true, message: $t(`['common']['deviceTypecannotNull']`) }],
        ...
        },
        {
          ...
          rules: [{ required: true, message: $t(`['common']['jumpTypecannotNull']`) }],
          component: 'radioGroup',
        ...
        },
        {
          prop: 'showType',
          label: '登录状态',
          rules: [{ required: true, message: $t(`['common']['loginStatusCannotNull']`) }],
          component: 'radioGroup',
          attrs: { options: showTypes.list }
        },
        ...(() => {
          ...
          return this.languages.list
            ...
            .map(language => {
              return {
                ...
                attrs: { props: { accept: '.png,.jpg,.jpeg', volume: [0, 300 * 1024], tips: '提示：图片尺寸540x720px，大小300K以内，格式 jpg\\jpeg\\png' }}
              }
            })
        })(),
        ...(() => {
          const { linkType, contentByLanguage } = this.formData

          // hide
          if (!linkType || linkType === 'noJump') {
  ...
  methods: {
    ...
    initFormData() {
      ...
      const { batchId, title, triggerTime, clientType, linkType, showType, languageList } = this.responseFormData

      this.formData.id = batchId
      this.formData.title = title
      this.formData.triggerTime = triggerTime
      this.formData.clientType = clientTypes.mapByValue.get(clientType).type
      this.formData.linkType = linkTypes.mapByValue.get(linkType).type
      this.formData.showType = showTypes.mapByValue.get(showType).type
    ...
    async getRequestFormData() {
      const { id: batchId, title, triggerTime, clientType, linkType, showType, contentByLanguage } = this.formData

      const formCommonData = {
        title,
        triggerTime,
        clientType: clientTypes.mapByType.get(clientType).value,
        linkType: linkTypes.mapByType.get(linkType).value,
        showType: showTypes.mapByType.get(showType).value,
